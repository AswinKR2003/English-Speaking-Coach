...
def init_db():
    ...
    # ------------------------------------------------------------------
    # users – added class_name and division columns
    # Roll number is unique only within a class+division combination.
    # The UNIQUE constraint is therefore on (roll_no, class_name, division).
    # ------------------------------------------------------------------
    c.execute('''
    ...
    # ---- migrate existing DB that may lack new columns ----
    ...
    # ---- Fix stale UNIQUE(roll_no) constraint on users table ----
    # Old databases may have UNIQUE(roll_no) alone, which prevents the same
    # roll number from existing in different classes/divisions.
    # We rebuild the table with the correct UNIQUE(roll_no, class_name, division).
    try:
        row = c.execute(
        ...
        if row:
            table_sql = row[0]
            # Check if old single-column unique on roll_no exists without the composite key
            has_old_unique = (
            ...
            if has_old_unique:
                ...
                c.execute('''
                    INSERT INTO users_new
                        (id, role, name, roll_no, class_name, division, email,
                ...
                print("users table rebuilt successfully with correct UNIQUE constraint.")
    except Exception as e:
        print(f"Migration note (users table rebuild): {e}")
...
# Helper: build a unique "student key" used across queries
def student_key(roll_no, class_name, division):
    """Returns a tuple that uniquely identifies a student."""
...
def detect_intent(text: str) -> str:
    ...
    # Special case: "how are you" and variations are greetings, not questions
...
1. GREETINGS (Hi, Hello, How are you, Good morning):
   - ALWAYS respond personally: "I'm doing great, thanks for asking!"
   - Be warm and friendly: "I'm happy to chat with you today!"
   - Example: 
     Child: "How are you?"
     ANSWER: I'm doing wonderful, thank you for asking! I'm excited to talk with you today.

2. QUESTIONS they ask YOU:
   - Answer their question directly and naturally
   - Example:
     Child: "Do you like cricket?"
     ANSWER: Yes, I think cricket is really exciting! I love watching the matches.
   - Example:
     Child: "What's your favorite color?"
     ANSWER: I really like blue! It reminds me of the sky and ocean. 

3. STATEMENTS they make:
   - React naturally with interest: "That's cool!" "Wow!" "Really?"
   - Share your thoughts on their topic
   - Example:
     Child: "I like playing football"
     ANSWER: Football is so much fun! It's great exercise and you get to run around a lot.

4. FEELINGS they share:
   - Acknowledge it warmly: "I'm so glad you're happy!"
   - Example:
     Child: "I am happy"
     ANSWER: That's wonderful! I'm so glad you're feeling happy today.

5. SHORT REPLIES (yes, no, okay, fine):
   - React naturally: "Great!" "Awesome!" "Okay!"
   - Keep the conversation flowing
   - Example:
     Child: "Yes"
     ANSWER: Great! I'm glad to hear that.

══════════════════════════════════════════════════
CRITICAL CONVERSATION RULES:
══════════════════════════════════════════════════
✓ ALWAYS respond to what they ACTUALLY said
✓ Be natural and friendly like a real person
✓ If they ask "how are you", tell them you're fine/great/wonderful
✓ If they talk about cricket, talk about cricket
✓ Show genuine interest in their topics
✓ Keep responses SHORT (1-2 sentences in ANSWER)
✗ NEVER ignore what they said
✗ NEVER give generic responses
✗ NEVER just repeat grammar rules

══════════════════════════════════════════════════
GRAMMAR CORRECTION:
══════════════════════════════════════════════════
- Fix ONE main grammar error (tense, pronoun, missing word, capitalization)
- If already correct, copy their sentence exactly
- Examples:
...
Child: "How are you?"
CORRECT: How are you?
ANSWER: I'm doing great, thank you so much for asking! I'm happy to talk with you.
PRAISE: Nice question!
QUESTION: How are you feeling today?

Example 2:
Child: "I am fine"
CORRECT: I am fine.
ANSWER: That's wonderful! I'm glad you're doing well.
PRAISE: Great!
QUESTION: What did you do today?

Example 3:
Child: "I played cricket"
CORRECT: I played cricket.
ANSWER: Oh wow, cricket is so much fun! I love how exciting the game is.
PRAISE: Cool!
QUESTION: Did you bat or bowl today?

Example 4:
Child: "Do you like mango?"
CORRECT: Do you like mango?
ANSWER: Yes, I think mangoes are delicious! They're so sweet and juicy.
...
══════════════════════════════════════════════════
CORRECT: <grammar-corrected sentence or original if already correct>
ANSWER: <natural, friendly response - 1-2 sentences>
PRAISE: <short encouragement>
QUESTION: <one follow-up question to continue conversation>"""
...
def _build_coach_messages(child_text: str, context: str, intent: str, topic: str) -> list[dict]:
    ...
    intent_hint_map = {
        "question":     "🔔 CRITICAL: Child is asking YOU a question. In ANSWER, respond directly as if YOU (the coach) are answering. Example: Child: 'How are you?' → ANSWER: 'I'm doing great, thanks for asking!'",
        "greeting":     "🔔 Child is greeting you (hi/hello/how are you). In ANSWER, greet them back warmly and tell them how YOU are. Example: 'I'm wonderful, thank you! Happy to chat with you today!'",
        "feeling":      "🔔 Child is sharing their feelings. In ANSWER, acknowledge their feeling warmly. Example: Child: 'I am happy' → ANSWER: 'That's wonderful! I'm so glad you're feeling happy!'",
        "short_answer": "🔔 Child gave a short reply (yes/no/okay). In ANSWER, react naturally and continue based on what was discussed before.",
        "statement":    "🔔 Child made a statement. In ANSWER, respond with interest about THEIR specific topic. Example: Child: 'I like cricket' → ANSWER: 'Cricket is so exciting! I love watching the matches too.'",
    }
    hint = intent_hint_map.get(intent, "")
    ...
    if topic:
        context_block += f"[Current topic being discussed: {topic}]\n\n"
    if context_block:
        messages.append({"role": "user", "content": context_block.strip()})

    user_content = f"{hint}\n\nChild says: {child_text}" if hint else f"Child says: {child_text}"
...
def english_coach(child_text):
    """
    Simplified English coach for children aged 6-15.
    Uses session storage instead of global variables (Flask best practice).
    """
    # Get conversation context from session (instead of global variable)
...
"""
    ...
    # Save context to session (instead of global variable)
    conversation_context += f"\nChild: {child_text}\nAssistant: {reply}"
    conversation_context = conversation_context[-1000:]  # Keep last 1000 chars
    set_conversation_context(conversation_context)
    
    # Return CoachResponse object (required by the /process route)
    return CoachResponse(
        corrected=fields["CORRECT"] or child_text,
        answer="",  # Not used in simplified version
...
_ROLEPLAY_ROLE_PROMPTS = {
    "teacher": (
        "You are a kind, enthusiastic school teacher.\n"
        "Your student is aged 6-15. Help them learn and grow.\n"
        "If they ask a question, answer it clearly and simply.\n"
        "If they say something, react as a real teacher would — encourage, build on what they said.\n"
        "Ask study-related or curiosity-building questions.\n"
        "Be encouraging and patient."
    ),
    "friend": (
        "You are a cheerful, friendly classmate the same age.\n"
        "Talk casually and happily like a real friend.\n"
        "If they ask you something, answer like a friend would — naturally and personally.\n"
        "React to what they say with genuine interest.\n"
        "Be playful, supportive, and relatable."
    ),
    "interviewer": (
        "You are a polite, professional job interviewer.\n"
        "If the candidate answers well, acknowledge it and build on it.\n"
        "If they ask a question, answer it professionally.\n"
        "Ask short, relevant interview questions.\n"
        "Be encouraging but maintain a professional tone."
    ),
    "viva": (
        "You are a fair, encouraging viva examiner.\n"
        "If the student explains something, react as an examiner — probe deeper or praise clarity.\n"
        "If they ask you a question, answer it helpfully.\n"
        "Focus on understanding, not just answers.\n"
        "Be polite, academic, and encouraging."
    ),
}
...
def _build_roleplay_messages(
    ...
    intent_hint_map = {
        "question":     f"⚠️ The student is ASKING YOU A QUESTION. Answer it in character as {roleplay_type}.",
        "greeting":     f"⚠️ The student is GREETING YOU. Respond warmly in character as {roleplay_type}.",
        "feeling":      f"⚠️ The student is expressing a FEELING. Acknowledge it as {roleplay_type} would.",
        "short_answer": f"⚠️ The student gave a SHORT REPLY. React naturally and stay in your {roleplay_type} role.",
        "statement":    f"⚠️ The student made a STATEMENT. Engage with it meaningfully as {roleplay_type}.",
    }
    intent_hint = intent_hint_map.get(intent, "")
...
- No grammar meta-explanation, just model good English

Here is a suggested question for context (use it or a natural variation):
"{suggested_question}"

Reply in this EXACT format — no extra text:
CORRECT: <corrected version of student's input>
ANSWER: <your in-character, direct response to what they said>
...
def roleplay_coach(
    ...
    *,
    ...
    try:
        ...
        logger.error(
            "Roleplay coach error [%s] for input %r: %s", roleplay_type, child_text, exc
        )
...
Example output for "brave": The brave girl saved the little cat.
Example output for "run": I like to run in the park.
Example output for "table": She put her book on the table."""
    try:
        response = client.chat.completions.create(
            model="llama-3.1-8b-instant",
            messages=[{"role": "user", "content": prompt}],
        ...
        sentence = re.sub(
            rf'^{re.escape(word.lower())}[\s:.\-–]+',
            '',
            sentence,
            flags=re.IGNORECASE
        ).strip()
        sentence = re.sub(
            r'^(sentence|example|output|answer|usage)\s*[:\-–]\s*',
            '',
            sentence,
            flags=re.IGNORECASE
        ).strip()
        ...
        if (
            ...
            or sentence.lower().strip('.!?') == word.lower()
            or word.lower() not in sentence.lower()
        ):
...
def login():
    ...
    if role == "student":
        ...
        if not roll_no or not password or not class_name or not division:
            conn.close()
            return jsonify({"success": False, "message": "Please provide roll number, class, division and password"})

        user = conn.execute(
            'SELECT * FROM users WHERE role=? AND roll_no=? AND class_name=? AND division=?',
            (role, roll_no, class_name, division)
        ).fetchone()
    else:
    ...
    if user and check_password_hash(user['password_hash'], password):
        ...
        return jsonify({"success": False, "message": "Invalid credentials. Check roll number, class, division and password."})
...
def signup():
    ...
    try:
        if role == "student":
            ...
            # Uniqueness: same roll_no in same class+division is not allowed
            # but same roll_no in a DIFFERENT class or division is perfectly fine
            existing = conn.execute(
            ...
            conn.execute(
                'INSERT INTO users (role, name, roll_no, class_name, division, password_hash) VALUES (?,?,?,?,?,?)',
                (role, name, roll_no, class_name, division, password_hash)
            )
            conn.execute(
...
def reset_password_request():
    ...
    if not roll_no or not new_password or not class_name or not division:
        return jsonify({"success": False, "message": "Roll number, class, division and new password are required"})
    if len(new_password) < 4:
        return jsonify({"success": False, "message": "Password must be at least 4 characters"})

    conn = get_db_connection()
    user = conn.execute(
        'SELECT * FROM users WHERE roll_no=? AND class_name=? AND division=? AND role=?',
        (roll_no, class_name, division, 'student')
    ).fetchone()
    if not user:
        conn.close()
        return jsonify({"success": False, "message": "No student found with that roll number, class and division"})

    conn.execute(
        'UPDATE users SET password_hash=? WHERE roll_no=? AND class_name=? AND division=?',
        (generate_password_hash(new_password), roll_no, class_name, division)
    )
    conn.commit()
    conn.close()
...
def delete_account():
    ...
    conn = get_db_connection()
    try:
        conn.execute('DELETE FROM activity_log WHERE roll_no=? AND class_name=? AND division=?',
                     (roll_no, class_name, division))
        conn.execute('DELETE FROM student_progress WHERE roll_no=? AND class_name=? AND division=?',
                     (roll_no, class_name, division))
        conn.execute('DELETE FROM student_badges WHERE roll_no=? AND class_name=? AND division=?',
                     (roll_no, class_name, division))
        conn.execute('DELETE FROM student_sessions WHERE student_id=?', (user_id,))
        conn.execute('DELETE FROM users WHERE roll_no=? AND class_name=? AND division=? AND role=?',
                     (roll_no, class_name, division, 'student'))
        conn.commit()
        conn.close()
        session.clear()
...
def check_repeat():
    ...
    return jsonify({
        ...
        "you_said": student,
        "correct_version": correct if needs_correction else None
    })
...
def get_leaderboard():
    ...
    my_rank_row = conn.execute('''
        ...
        WHERE xp > (
            SELECT xp FROM student_progress
...
def get_student_info():
    ...
    student = conn.execute(
        'SELECT name, roll_no, class_name, division FROM users WHERE roll_no=? AND class_name=? AND division=?',
        (roll_no, class_name, division)
    ).fetchone()
...
def update_xp():
    ...
    if progress:
        ...
        newly_earned_badge_ids = check_earned_badges(
            ...
            mode=mode, difficulty=difficulty, score=score, stars_earned=stars_earned
        )
...
def get_all_students():
    ...
    students = conn.execute('''
        SELECT u.name, u.roll_no, u.class_name, u.division,
...
def get_student_details(roll_no):
    # Accept optional class/division via query params for disambiguation
...
def teacher_reset_student_password():
    ...
    if not roll_no or not new_password or not class_name or not division:
        return jsonify({"success": False, "message": "Roll number, class, division and new password required"})
    conn = get_db_connection()
    user = conn.execute(
        'SELECT * FROM users WHERE roll_no=? AND class_name=? AND division=? AND role=?',
        (roll_no, class_name, division, 'student')
